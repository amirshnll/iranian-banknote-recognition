    ...
    if len(set(att)) == 1:
        ...
        for i in range(1, len(cat)):
            if not att[i] == att[i - 1]:
                gains.append(
                    entropy(cat[:i]) * float(i) / len(cat)
                    + entropy(cat[i:]) * (1 - float(i) / len(cat))
                )
                div_point.append(i)
        ...
        if max(division) == 0:
            ...
            if isnum(data[index_selected]):
                ...
                if len(l_son_category) > 0 and len(r_son_category) > 0:
                    p_l = float(len(l_son_category)) / (
                        len(data[index_selected]) - data[index_selected].count("?")
                    )
                    son = ET.SubElement(
                        parent,
                        name_selected,
                    ...
                    if not k == "?":
                        ...
                        son = ET.SubElement(
                            ...
                            name_selected,
                            {
                                ...
                                "p": str(
                                    round(
                                        float(len(son_category))
                                        / (
                                            len(data[index_selected])
            ...
            if att == "?":
                ...
                for child in root.childNodes:
                    d = add(
                        d,
                        decision(
                            child, obs, attrs_names, p * float(child.getAttribute("p"))
                        ),
                    )
                return d
            else:
                for child in root.childNodes:
                    if (
                        child.getAttribute("flag") == "m"
                        and child.getAttribute("value") == att
                        or child.getAttribute("flag") == "l"
                        and float(att) < float(child.getAttribute("value"))
                        or child.getAttribute("flag") == "r"
                        and float(att) >= float(child.getAttribute("value"))
                    ):
